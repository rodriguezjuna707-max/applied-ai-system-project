...
class Song:
    ...
    popularity: float = 50.0        # 0–100  streaming popularity score
    release_decade: str = "2020s"   # e.g. "2010s"  era of the track
    mood_tag: str = ""              # detailed tag: euphoric / serene / gritty / …
    instrumentalness: float = 0.10  # 0.0–1.0  fraction without vocals
    liveness: float = 0.15          # 0.0–1.0  live-recording feel
...
class Recommender:
    ...
    def recommend(self, user: UserProfile, k: int = 5) -> List[Song]:
        ...
        scored = sorted(
            self.songs,
            key=lambda s: score_song(user_dict, _song_to_dict(s))[0],
            reverse=True,
        )
        return scored[:k]
    ...
    def explain_recommendation(self, user: UserProfile, song: Song) -> str:
        user_dict = _user_to_dict(user)
        _, reasons = score_song(user_dict, _song_to_dict(song))
        return "; ".join(reasons)
